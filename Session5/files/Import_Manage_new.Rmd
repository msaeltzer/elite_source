...
mdb4<-r1$DOCUMENT[[4]]$BIOGRAFISCHE_ANGABEN$GEBURTSDATUM

mdb4$WAHLPERIODEN[[1]]$INSTITUTIONEN[[1]]$INS_LANG
...
print(r2[[i]]$NAMEN$NAME$NACHNAME[[1]])
}
...
places<-c()
for(i in 1:length(r2)){
if(length(r2[[i]]$NAMEN$NAME$ORTSZUSATZ)==0){ # if a subscript is out of bounds, it means the index you referred to doesnt exist. The list is shorter than the index. If the list has lenght 0 we instead place an NA in the respective vector position
places[[i]]<-NA
}else
places[i]<-r2[[i]]$NAMEN$NAME$ORTSZUSATZ[[1]]
}
